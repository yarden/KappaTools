          ...
          let error,log_info,causal_table =
            ...
            then
              ...
              let log_info,error =
                ...
                then
                  let error,log_info,() =
                    U.export_musical_grid_to_xls parameter handler log_info error "a" 0 0 blackboard
                  in
              ...
              let error,log_info,list =
                U.extract_observable_hits_from_musical_notation parameter handler log_info error blackboard
              in
              ...
              let error,log_info,causal_story_list =
                ...
                let error,log_info,enriched_grid =
                  U.enrich_grid_with_transitive_past_of_each_node_without_a_progress_bar parameter handler log_info error grid
                in
                let () =
                  ...
                  (fun
                    ...
                    let error,log_info,trace_before_compression =
                      U.causal_prefix_of_an_observable_hit
                        ...
                        "compression_main, line 2014"
                        blackboard enriched_grid observable_id
                    in
                    let info =
    ...
    if causal_trace_on then
      ...
      let error,log_info =
        Causal.pretty_print
          ...
          log_info error env Graph_closure.config_small_graph "" "" export
      in
      error,log_info
    else error,log_info
  in
  let error,log_info =
    if weak_compression_on then
      ...
      let error,log_info =
        Causal.pretty_print
          ~dotFormat (S.PH.B.PB.CI.Po.K.H.get_kasa_parameters parameter) handler
          log_info error  env Graph_closure.config_small_graph "Weakly" "weakly " export
      in
      error,log_info
    else error,log_info
  in
  let error,_log_info =
    if strong_compression_on then
      ...
      let error,log_info =
        Causal.pretty_print
          ~dotFormat (S.PH.B.PB.CI.Po.K.H.get_kasa_parameters parameter) handler
          log_info error env Graph_closure.config_small_graph "Strongly" "strongly " export
      in
      error,log_info
    else
      error,log_info
